"""
ResNet with Instance-Batch Normalization (IBN) for Vehicle Re-ID
Based on "Two at Once: Enhancing Learning and Generalization Capacities via IBN-Net"
"""
...
class IBN(nn.Module):
    ...
    def __init__(self, planes, ratio=0.5):
        ...
        self.half_channels = int(planes * ratio)
        self.BN = nn.BatchNorm2d(planes - self.half_channels)
        self.IN = nn.InstanceNorm2d(self.half_channels, affine=True)

    def forward(self, x):
        split = torch.split(x, self.half_channels, 1)
...
def resnet50_ibn_a(pretrained=False, **kwargs):
    ...
    model = ResNet_IBN(Bottleneck, [3, 4, 6, 3], ibn_cfg=('a', 'a', 'a', None), **kwargs)
    return model